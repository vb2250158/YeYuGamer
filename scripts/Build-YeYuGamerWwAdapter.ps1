...
$wwAccountBridgeSource = Assert-LocalFile (Join-Path $source 'adapter-host\openkuro-runner\WwAccountYeYuBridge.py') 'WW saved-account bridge source'
...
$wwAccountBridge = Join-Path $candidate 'WwAccountYeYuBridge.py'
Copy-Item -LiteralPath $wwAccountBridgeSource -Destination $wwAccountBridge
...
$manifest.files += [ordered]@{path='WwAccountYeYuBridge.py';sha256=(Hash $wwAccountBridge);sizeBytes=(Get-Item -LiteralPath $wwAccountBridge).Length}
