...
一键每日的范围等于：**开启游戏滑轨且至少选中一个每日步骤的游戏**。按游戏卡片顺序依次执行；每个游戏只收到用户选中的步骤。
...
- 每个游戏正常结束时，先回收本轮 Adapter/工具进程树，再关闭本轮由 Manager 启动的游戏客户端。每次启动下一款游戏前，Manager 依据当前未封存执行批次的冻结游戏范围，通过 `close_for_queue` 关闭其中其他已配置游戏的客户端，包括队列启动前已存在的客户端；独立 GameRun 不取得关闭其他游戏的权限。
