...
const props = defineProps<{ modelValue: GameAccount[]; disabled: boolean }>()
...
function add(): void {
  if (props.disabled) return
  emit('update:modelValue', [...props.modelValue, { label: `账号 ${props.modelValue.length + 1}`, enabled: true, saved_account_label: '' }])
}
</script>
...
<template>
  <section class="account-editor">
    ...
    <p class="soft-note">同一客户端按下方顺序执行，所有账号共用下面的每日选择和参数。仅填写官方登录页已记住账号的掩码标签，不填写密码或验证码。保存后由 Manager 分配账号 ID；已有账号可停用，历史记录保留。</p>
    <p class="soft-note">已有执行记录的账号请勿更换登录标签；换号请新增账号并停用旧项。首次配置多账号时，若“当前账号”已有执行记录，请停用它，再分别添加已记住的账号，避免沿用旧号的完成记录。</p>
    <div v-for="(account, index) in modelValue" :key="account.account_id ?? `new-${index}`" class="account-row">
